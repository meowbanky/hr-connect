...
try {
    // Basic mapping for redundancy if needed, though we primarily use IDs if schema says type_id
    // But schema says `employment_type` ENUM too. We should try to fetch the name from ID or just set it if we have it.
    // Let's look up the employment type name to fill the enum column
    $empTypeEnum = '';
    if ($employment_type_id) {
        ...
        // Note: Enum values must match EXACTLY: 'Full-time','Part-time','Contract','Internship'
    }
    ...
    $stmt->execute([
        ...
        $location, $min_salary, $max_salary, $salary_range,
        $description, $requirements, $experience_level,
        $close_date, $open_date, $status, $_SESSION['user_id']
    ]);

    echo json_encode(['success' => true, 'message' => 'Job created successfully', 'id' => $pdo->lastInsertId()]);
